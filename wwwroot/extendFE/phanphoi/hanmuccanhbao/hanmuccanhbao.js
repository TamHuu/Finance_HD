...
$(document).ready(function () {
    table = $('#Table').DataTable({
        columnDefs: [
            ...
            { width: '400px', orderable: false, className: 'dt-left dt-head-center', targets: [1] },
            { width: '150px', orderable: false, className: 'dt-right dt-head-center', targets: [2] },
            { width: '50px', orderable: false, targets: [3, 4], className: 'text-center' },
        ],
...
function loadDanhSach() {
    $.ajax({
        url: "/WarningLimit/getListWarningLimit",
        method: "GET",
...
function drawDanhSach(data) {
    ...
    data.forEach(function (item, index) {
        let ma = item.ma;
        let tenbophan = item.tenBoPhan;
        let hanmuc = item.hanMuc;
...
function Edit(row) {
    ...
    window.open('/WarningLimit/Edit?ma=' + firstCellValue, '_blank');
