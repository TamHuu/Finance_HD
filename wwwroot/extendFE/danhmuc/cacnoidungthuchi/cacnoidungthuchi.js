...
$(document).ready(function () {
    table = $('#Table').DataTable({
        columnDefs: [
            ...
            { width: '600px', className: 'dt-left dt-head-center', targets: [1, 2,3,4], orderable: false },
...
function loadDanhSach() {
    $.ajax({
        url: "/IncomeContent/getListIncomeContent",
        method: "GET",
...
function drawDanhSach(data) {
    ...
    data.forEach(function (item, index) {   
        ...
        let code = item.code;
        let ten = item.ten;
        let loaithuchi = item.tenLoaiThuChi;
        let noibo = item.noiBo == true ? 1 : 0;;
...
function Edit(row) {
    ...
    window.open('/IncomeContent/Edit?ma=' + firstCellValue, '_blank');
