...
$(document).ready(function () {
    table = $('#Table').DataTable({
        columnDefs: [
            { width: '200px', className: 'dt-left dt-head-center', targets: [0,1], orderable: false },
            { width: '200px', className: 'dt-left dt-head-center', targets: [2, 3,4,5,6], orderable: false },
        ],
...
function loadDanhSach() {
    $.ajax({
        url: "/LoginHistory/getListLoginHistory",
        method: "GET",
...
function drawDanhSach(data) {
    ...
    data.forEach(function (role, index) {
        ...
        let code = role.code;
        let ten = role.name;
        let mota = role.description??"";
        let tentrangthai = role.status == true ? "Hoạt động" : "Hết hoạt động";
        let rowContent = [
            `<td>${ma}</td>`,
            `<td>${code}</td>`,
            `<td>${ten}</td>`,
...
function Edit(row) {
    ...
    window.open('/LoginHistory/Edit?ma=' + firstCellValue, '_blank');
