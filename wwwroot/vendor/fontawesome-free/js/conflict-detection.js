  ...
  function report (_ref) {
    ...
    if (conflictsCount > 0) {
      ...
      for (var _key in nodesTested.conflict) {
        var product = nodesTested.conflict[_key];
        data[_key] = {
          'tagName': product.tagName,
          'src/href': product.src || product.href || 'n/a',
          'innerText excerpt': product.innerText && product.innerText !== '' ? product.innerText.slice(0, 200) + '...' : '(empty)'
        };
      }
    ...
    if (noConflictsCount > 0) {
      ...
      for (var _key2 in nodesTested.noConflict) {
        var _product = nodesTested.noConflict[_key2];
        _data[_key2] = {
          'tagName': _product.tagName,
          'src/href': _product.src || _product.href || 'n/a',
          'innerText excerpt': _product.innerText && _product.innerText !== '' ? _product.innerText.slice(0, 200) + '...' : '(empty)'
        };
      }
    ...
    if (timeOutCount > 0) {
      ...
      for (var _key3 in timedOutTests) {
        var _product2 = timedOutTests[_key3];
        _data2[_key3] = {
          'tagName': _product2.tagName,
          'src/href': _product2.src || _product2.href || 'n/a',
          'innerText excerpt': _product2.innerText && _product2.innerText !== '' ? _product2.innerText.slice(0, 200) + '...' : '(empty)'
        };
      }
